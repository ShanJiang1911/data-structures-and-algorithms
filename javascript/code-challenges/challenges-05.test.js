...
const addValues = (arr) => {
  ...
  let initiaValue = 0;
  return arr.reduce(reducer, initiaValue);
  // Solution code here...
};
...
const addPurchases = (arr) => {
  // let reducer = [arr.purchasePrice];
  let total = arr.reduce(
    (accumulator, currentvalue) => accumulator + currentvalue.purchasePrice, 0);
  // let purchaseSum = addPurchases.reduce( function(accumulator, value, idx) {
...
const countNumberOfElements = (arr) => {
  let totalCount = arr.reduce( (acc) => (acc + 1), 0);
...
const returnNames = (arr) => {
  let newName = [];
    ...
    newName.push(currentValue.name),[]);
    return newName;
  // return newName;
  // Solution code here...
};
